   ...
   if (pairingCode && !GlobalTechInc.authState.creds.registered) {
      ...
      if (!!phoneNumber) {
         ...
         if (!Object.keys(PHONENUMBER_MCC).some(v => phoneNumber.startsWith(v))) {
            console.log(chalk.bgBlack(chalk.redBright("Comece com o código do país do seu número do WhatsApp, Exemplo : +555189926591")))
            process.exit(0)
         }
      } else {
         phoneNumber = await question(chalk.bgBlack(chalk.greenBright(`Por favor, digite seu número do WhatsApp 😍\nFor exemplo: +555189926591 : `)))
         phoneNumber = phoneNumber.replace(/[^0-9]/g, '')
...
GlobalTechInc.ev.on("connection.update",async  (s) => {
        ...
        if (connection == "open") {
	        ...
	        console.log(chalk.magenta(`\n${themeemoji} YT CHANNEL: @RicZin 404`))
            console.log(chalk.magenta(`${themeemoji} GITHUB: RicZin `))
